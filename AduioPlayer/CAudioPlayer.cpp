...
int CAudioPlayer::stream_component_open(){
    AVDictionaryEntry *t = NULL;
    int64_t wanted_channel_layout = 0;

    if (audioStream < 0 || audioStream >= static_cast<int>(pFormatCtx->nb_streams))
        return ERR_STREAM_NOT_FOUND;
    pCodecCtx = pFormatCtx->streams[audioStream]->codec;
    pCodec = avcodec_find_decoder(pCodecCtx->codec_id);
    if (!pCodec)
        return ERR_UNSUPPORT_CODEC;
    ...
    if (pCodecCtx->codec_type == AVMEDIA_TYPE_AUDIO) {
        ...
        if (wanted_spec.freq <= 0 || wanted_spec.channels <= 0) {
            ...
            return ERR_INVALID_RATE_CH;
        }
    }

    if (!pCodec ||
        avcodec_open(pCodecCtx, pCodec) < 0)
        return ERR_OPEN_CODEC;
...
void CAudioPlayer::stream_component_close(){
    if (audioStream < 0 || audioStream >= static_cast<int>(pFormatCtx->nb_streams))
        return;
    AbortPacketQueue();
    SDL_CloseAudio();
...
int CAudioPlayer::PlayThread(void* param){
	CAudioPlayer* p = (CAudioPlayer*)param;
	p->status = PLAY;
	AVPacket pkt;
	while (!p->queue.abort_request){
		if (p->queue.size > (20 * 16 * 1024)){
			SDL_Delay(10);
			continue;
		}
...
int CAudioPlayer::Open(const char* filename){
	...
	pCodecCtx = pFormatCtx->streams[audioStream]->codec;
	int ret;
	if ((ret = stream_component_open()) != 0){
		Close();
		return ret;
	}
	pFormatCtx->interrupt_callback.callback = decode_inte;
...
void CAudioPlayer::InitPacketQueue(){
	PacketQueue *q = &queue;
...
void CAudioPlayer::FlushPacketQueue()
{
	PacketQueue *q = &queue;
...
int CAudioPlayer::PutPacket(AVPacket* pkt){
	PacketQueue *q = &queue;
	AVPacketList *pkt1;
...
void CAudioPlayer::EndPacketQueue(){
	PacketQueue* q = &queue; 
    FlushPacketQueue();
    SDL_DestroyMutex(q->mutex);
    SDL_DestroyCond(q->cond);
}
void CAudioPlayer::AbortPacketQueue(){
	PacketQueue* q = &queue; 
    SDL_LockMutex(q->mutex);
    q->abort_request = 1;
    SDL_CondSignal(q->cond);
...
int CAudioPlayer::GetPacket(AVPacket *pkt, int block){
	PacketQueue *q = &queue;
	AVPacketList *pkt1;
    int ret;
...
void CAudioPlayer::CallBack(void *opaque, Uint8 *stream, int len){
	CAudioPlayer* p = (CAudioPlayer*)opaque;
	VideoState *is = &p->as;
    ...
    while (len > 0) {
		...
		if (len1 >= 2){
			is->l_amplitude = (int16_t)*((int16_t*)(is->audio_buf + is->audio_buf_index));
			is->r_amplitude = (int16_t)*((int16_t*)(is->audio_buf + is->audio_buf_index + 2));

			is->l_dB = int(20 * log10( abs(is->l_amplitude) / 32767.0 ));
			is->r_dB = int(20 * log10( abs(is->r_amplitude) / 32767.0 ));

			//cout<<len1<<"          "<<endl;
			if (p->pSpectrumAnalyzer && len1 >= 1024 && !is->paused && p->status != FINISHED)
				p->pSpectrumAnalyzer->PushData((int16_t*)(is->audio_buf + is->audio_buf_index), 1024);
		}

		SDL_MixAudio(stream, (uint8_t *)is->audio_buf + is->audio_buf_index, len1, 0);//p->volume);
        len -= len1;
        stream += len1;
...
int CAudioPlayer::AudioDecodeFrame(double *pts_ptr){
	VideoState* is = &as;
